...
def split_nodes_image(old_nodes):
    return split_node_internal(old_nodes,r"(!\[([^\[\]]*)\]\(([^\(\)]*)\))",TextType.IMAGE)

def split_nodes_link(old_nodes):
    return split_node_internal(old_nodes,r"((?<!!)\[[^\[\]]*\]\([^\(\)]*\))",TextType.LINK)
        ...
        for split_line in split_result:
            if len(split_line)>0:
                if re.match(pattern,split_line)!=None:
                    ...
                    result.append(TextNode(split_line,TextType.TEXT))
    return result
